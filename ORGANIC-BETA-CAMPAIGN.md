# DINAVO – kostenlose organische Beta-Kampagne

Stand: 02.09.2026

Status: **aktiv – erster DINAVO-Beitrag auf Facebook und LinkedIn veröffentlicht; weitere Beiträge geplant**

Veröffentlichte DINAVO-Beiträge:

- Facebook: https://facebook.com/122102831157445036/posts/122107501845445036
- LinkedIn: https://linkedin.com/feed/update/urn:li:share:7500684328297717760

Die aktive Planung verwendet ausschließlich DINAVO, saubere Cloudflare-Links ohne eigene Trackingparameter und den kostenlosen Metricool-Rahmen. Frühere DealFaz-Entwürfe bleiben inaktiv und werden nicht veröffentlicht.

Ziel der ersten sieben Tage: 20 echte Rechner-Tests und mindestens 5 verständliche Rückmeldungen. Es gibt kein Umsatz-, Reichweiten- oder Besucherzahlversprechen.

## Sichere Kampagnengrenzen

- ausschließlich organische, unbezahlte Beiträge;
- DINAVO klar als eigene kostenlose Beta bezeichnen;
- keine Affiliate- oder Partnerlinks, keine Provisionen und kein Checkout;
- keine Gewinn-, Verkaufs-, Preis- oder Nachfragegarantie;
- keine Aussage, DINAVO sei eine eingetragene oder weltweit freie Marke;
- keine gekauften Follower, keine Bots, keine Massen-DMs und kein Gruppen-Spam;
- keine eigenen Trackinglinks: beworben wird nur die saubere Hauptadresse;
- Reichweite, Wiedergaben und Klicks nur als Plattformwerte bezeichnen, nicht als eindeutige Menschen.

Öffentliche Hauptadresse:

> https://dealfaz.dealfaz-social.workers.dev/

## Profile vorbereiten

**Anzeigename**

> DINAVO · Deal-Check

**Kurze Bio**

> Reselling-Deals kostenlos prüfen  
> Gewinn · ROI · Gebühren  
> Beta · ohne Anmeldung

**Link im Profil**

> https://dealfaz.dealfaz-social.workers.dev/

Bestehende Handles wie `@dealfaz` dürfen vorerst als technische Legacy-Adresse bestehen bleiben. Ein öffentlicher Handle-Wechsel wird erst nach Verfügbarkeitsprüfung und ausdrücklicher Freigabe durchgeführt.

## Einheitlicher Aufbau jedes Beitrags

1. Eine konkrete Frage oder ein konkreter Rechenfehler als Einstieg.
2. Eine nachvollziehbare Beispielrechnung auf dem Bildschirm.
3. Der Hinweis, dass alle Werte Annahmen sind.
4. Genau eine CTA: `Kostenlos prüfen – Link im Profil.`
5. Abschluss: `Eigene kostenlose Beta. Keine Gewinn- oder Verkaufsgarantie.`

## Sieben fertige Beiträge

### Tag 1 – Was bleibt wirklich übrig?

**Video:** `50 € Einkauf → 100 € Verkauf` eingeben, dann Gebühren und Ergebnis öffnen.

**Sprechertext:**

> 50 Euro einkaufen und für 100 Euro verkaufen klingt nach 50 Euro Gewinn. Aber Gebühren, Versand und weitere Kosten fehlen noch. DINAVO rechnet alles zusammen. In diesem Beispiel bleiben rechnerisch 28,66 Euro. Kostenlos prüfen – Link im Profil.

**Caption:**

> Einkauf und Verkauf allein reichen nicht. DINAVO zeigt Gewinn, ROI und Kosten in einer Rechnung. Eigene kostenlose Beta. Keine Gewinn- oder Verkaufsgarantie. #reselling #kleinanzeigen #flipping #dinavo

### Tag 2 – Plattformgebühren

**Video:** Zwischen zwei Gebührenprofilen wechseln und zeigen, wie sich der Gewinn ändert.

**Sprechertext:**

> Derselbe Verkaufspreis kann je nach Plattform ein anderes Ergebnis haben. DINAVO übernimmt editierbare Gebührenprofile, damit du nicht mit einer versteckten Null rechnest.

**Caption:**

> Gebührenprofile sind Startannahmen und bleiben editierbar. Prüfe die aktuellen Plattformbedingungen immer selbst. Kostenlos testen – Link im Profil. #resellingtipps #ebay #vinted #dinavo

### Tag 3 – Weitere Kosten

**Video:** Verpackung, Reinigung und Fahrt als weitere Kosten ergänzen.

**Sprechertext:**

> Verpackung, Reinigung und Fahrt wirken einzeln klein. Zusammen können sie den Deal deutlich verändern. DINAVO zeigt sie getrennt und als Gesamtkosten.

**Caption:**

> Kleine Kosten sind auch Kosten. Kostenloser Beta-Rechner ohne Anmeldung. Keine Gewinn- oder Verkaufsgarantie. #resellingdeutschland #kosten #dinavo

### Tag 4 – Gewinn ist nicht ROI

**Video:** Gewinn und ROI nebeneinander markieren.

**Sprechertext:**

> 30 Euro Gewinn sagen noch nicht, wie effizient dein eingesetztes Geld arbeitet. Deshalb zeigt DINAVO Gewinn und ROI getrennt.

**Caption:**

> Gewinn und ROI beantworten zwei verschiedene Fragen. DINAVO erklärt beide direkt am Ergebnis. Kostenlos prüfen – Link im Profil. #roi #reselling #dinavo

### Tag 5 – Maximaler Einkaufspreis

**Video:** `Alle Zahlen ansehen` öffnen und den maximalen Einkauf zeigen.

**Sprechertext:**

> Statt nur zu fragen, was du verdienen könntest, kannst du rückwärts rechnen: Wie viel darfst du höchstens einkaufen, damit dein Ziel-ROI rechnerisch bleibt?

**Caption:**

> Vor dem Kauf eine Grenze setzen. Die Rechnung ersetzt keine Marktprüfung. Kostenlos testen – Link im Profil. #dealcheck #resellingtipps #dinavo

### Tag 6 – Daten bleiben im Browser

**Video:** Einen Deal merken und den Bereich `Gemerkte Deals` öffnen.

**Sprechertext:**

> DINAVO braucht kein Konto. Gemerkte Deals, Regeln und Ergebnisse bleiben in dieser Version lokal in deinem Browser.

**Caption:**

> Kostenlos, ohne Anmeldung und ohne DINAVO-eigenes Werbetracking. Lokale Browserdaten gehen verloren, wenn du sie oder deinen Browser löschst. #privacy #resellingtool #dinavo

### Tag 7 – Ehrliches Feedback

**Video:** Startseite, Rechner und Ergebnis jeweils kurz zeigen.

**Sprechertext:**

> DINAVO ist in der kostenlosen Beta. Teste einen echten oder vergangenen Deal und schreib ehrlich: Was war sofort klar und was war verwirrend?

**Caption:**

> Ich suche echte Rückmeldungen, keine Schönfärberei. Was fehlt dir vor einer Kaufentscheidung? Eigene kostenlose Beta. Link im Profil. #buildinpublic #reselling #dinavo

## Story-Abfolge für Tag 1

1. `50 € einkaufen. 100 € verkaufen. 50 € Gewinn?`
2. `Nicht automatisch: Gebühren, Versand und weitere Kosten fehlen.`
3. `DINAVO-Beispiel: 28,66 € rechnerischer Gewinn · 57,3 % ROI.`
4. `Kostenlos prüfen · Link im Profil.`

## Antworten auf typische Kommentare

**„Garantiert das den Verkauf?“**

> Nein. DINAVO rechnet deine Annahmen transparent durch, garantiert aber weder Preis noch Nachfrage oder Verkauf.

**„Woher kommen die Preise?“**

> Verkaufspreis und Vergleichswerte gibst du selbst ein. DINAVO kopiert keine fremde Marktdatenbank.

**„Sind die Gebühren immer aktuell?“**

> Die Profile sind editierbare Startannahmen. Vor dem Kauf solltest du die aktuellen, kategorieabhängigen Bedingungen der Plattform prüfen.

**„Speichert ihr meine Deals?“**

> In dieser Beta bleiben gemerkte Deals lokal in deinem Browser. DINAVO führt dafür keine zentrale Nutzerdatenbank.

## Kostenlose Auswertung

Nach jedem Beitrag nur diese Plattformwerte notieren:

| Tag | Plattform | Reichweite | Wiedergaben | Linkklicks | Kommentare | konkrete Rückmeldungen |
|---|---|---:|---:|---:|---:|---:|
| 1 |  |  |  |  |  |  |
| 2 |  |  |  |  |  |  |
| 3 |  |  |  |  |  |  |
| 4 |  |  |  |  |  |  |
| 5 |  |  |  |  |  |  |
| 6 |  |  |  |  |  |  |
| 7 |  |  |  |  |  |  |

## Vor jedem öffentlichen Beitrag

- Plattform und Zielkonto stimmen.
- Video und Caption zeigen DINAVO, nicht frühere Markennamen.
- Nur die saubere Hauptadresse wird verlinkt.
- Keine sensiblen Daten sind im Bildschirmvideo sichtbar.
- Keine Affiliate-Parameter oder bezahlte Kooperation sind enthalten.
- Beispielzahlen werden als Rechnung, nicht als echter Verdienst dargestellt.
- Der endgültige Beitrag wird erst nach ausdrücklicher Veröffentlichungsfreigabe abgesendet.
